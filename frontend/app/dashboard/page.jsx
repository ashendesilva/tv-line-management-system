...
export default function DashboardPage() {
    ...
    const fetchUsers = useCallback(async () => {
        ...
        try {
            ...
            const sorted = (res.data.users || []).slice().sort((a, b) => (b.is_active ? 1 : 0) - (a.is_active ? 1 : 0));
            setUsers(sorted);
    ...
    const partialCount = users.filter((u) => u.payment_status === 'partial').length;
    const unpaidCount = users.filter((u) => u.payment_status === 'unpaid').length;
    const activeCount = users.filter((u) => u.is_active).length;
    const todayPendingCount = users.filter((u) => u.monthly_payment_day === new Date().getDate() && u.payment_status !== 'paid').length;
    const todayCount = users.filter((u) => u.monthly_payment_day === new Date().getDate()).length;
    ...
    const handleViewPayments = (user) => setPaymentTarget(user);
    const handleClosePayments = () => setPaymentTarget(null);
    const handlePaymentChange = (updatedUser) => {
        setUsers((prev) => prev.map((u) => (u.id === updatedUser.id ? { ...u, ...updatedUser } : u)));
        setPaymentTarget((prev) => (prev && prev.id === updatedUser.id ? { ...prev, ...updatedUser } : prev));
    };

    // Legacy quick-pay (kept for compatibility, opens payment modal instead)
    const handlePay = (id) => {
        const user = users.find((u) => u.id === id);
        if (user) setPaymentTarget(user);
    };
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                    ...
                    <StatCard label="Partial" value={partialCount} color="amber" icon="½" />
                    <StatCard label="Unpaid" value={unpaidCount} color="red" icon="❌" />
                    <StatCard label="Today Total Bills" value={todayCount} color="blue" icon="📅" />
                    <StatCard label="Today Pending Bills" value={todayPendingCount} color="green" icon="📅" />
                    <StatCard label="Active" value={activeCount} color="indigo" icon="📡" />
                    <StatCard label="Inactive" value={totalCount - activeCount} color="red" icon="⛔" />
